...
func (ds *DataSource) GroupMessageCounts(ctx context.Context) (map[string]int64, error) {
	...
	if err != nil { return result, nil }
	var rooms []string
	for urows.Next() { var u string; if err := urows.Scan(&u); err == nil { rooms = append(rooms, u) } }
	urows.Close()
	...
	for _, db := range dbs {
		for _, uname := range rooms {
			...
			var cnt int64
			if err := db.QueryRowContext(ctx, q).Scan(&cnt); err == nil { result[uname] += cnt }
		}
	}
	return result, nil
}
...
func (ds *DataSource) IntimacyBase(ctx context.Context) (map[string]*model.IntimacyBase, error) {
	...
	for _, db := range dbs {
		...
		if err == nil {
			for rows.Next() { var name string; if rows.Scan(&name) == nil { tables = append(tables, tbl{db: db, name: name}) } }
			rows.Close()
		}
	}
	for _, t := range tables {
		...
		var v sql.NullInt64
		if row.Scan(&v) == nil && v.Valid && v.Int64 > maxCT { maxCT = v.Int64 }
	}
	...
	for _, t := range tables {
		...
		if !strings.HasPrefix(t.name, "Msg_") { continue }
		md5hex := strings.TrimPrefix(t.name, "Msg_")
		talker := md5ToUser[md5hex]
		if talker == "" { continue }
		if strings.HasSuffix(talker, "@chatroom") { continue }
		...
		if row.Scan(&total, &sent, &minct, &maxct) == nil {
			...
			if total.Valid { base.MsgCount += total.Int64; base.ReceivedCount += (total.Int64 - func() int64 { if sent.Valid { return sent.Int64 }; return 0 }()) }
			if sent.Valid { base.SentCount += sent.Int64 }
			if minct.Valid { if base.MinCreateUnix == 0 || minct.Int64 < base.MinCreateUnix { base.MinCreateUnix = minct.Int64 } }
			if maxct.Valid { if maxct.Int64 > base.MaxCreateUnix { base.MaxCreateUnix = maxct.Int64 } }
		}
...
func (ds *DataSource) GroupTodayMessageCounts(ctx context.Context) (map[string]int64, error) {
	...
	for _, db := range dbs {
		for _, uname := range rooms {
			...
			var cnt int64
			if err := db.QueryRowContext(ctx, q, since).Scan(&cnt); err == nil { result[uname] += cnt }
		}
	}
	return result, nil
}
...
func (ds *DataSource) GroupWeekMessageCount(ctx context.Context) (int64, error) {
	...
	for _, db := range dbs {
		for _, uname := range rooms {
			...
			tbl := "Msg_" + hex.EncodeToString(md5sum[:])
			var name string
			if err := db.QueryRowContext(ctx, `SELECT name FROM sqlite_master WHERE type='table' AND name=?`, tbl).Scan(&name); err != nil { continue }
			q := fmt.Sprintf(`SELECT COUNT(*) FROM %s WHERE create_time >= ?`, tbl)
			var cnt int64
			if err := db.QueryRowContext(ctx, q, since).Scan(&cnt); err == nil { total += cnt }
		}
	}
	return total, nil
}
